...
#
#            for k in range(r - 1): <--- use this one since index starts at 0
#            for k in range(r):
#            for k in range(r + 1):
#
...
#
#              for k in range(r - s):  <--- total = 0
#                  count = 0           <--- for k in range(r,s+1)
#                  if math.cos(r) > 0: <---     if math.cos(k) > 0
#                      count = 1       <---         total += math.cos(k)
#                  return count        <---     return total
#
#     f. The code in the "graphics accumulation" example below includes:
#            for _ in range(n):
#        What does the   _   (underscore) mean?
#           It represents the variable that contains the numerical value that is
#           incremented each time the loop is completed. This variable can be used
#           within the loop, since that is where it exists. Often times, "k" is used
...
#
#              x = starting_point.x                      <--- this approach is wrong
#              for k in range(n):                             because it does not adjust
#                  center = rg.Point(x + (k * diameter), y)   the value of x in any way
#                  circle = rg.Circle(point, radius)
#                  circle.attach_to(window)
#
#              x = starting_point.x                      <--- this attempt is correct
#              for k in range(n):
